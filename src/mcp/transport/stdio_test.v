...
fn test_read_stdin_message_parses_content_length_frame() {
	...
	mut reader := io.new_buffered_reader(reader: FramedReader{ text: frame })
...
fn test_read_stdin_message_parses_multiple_framed_messages() {
	...
	stream := ['Content-Length: ${init.len}\r\n\r\n${init}', 'Content-Length: ${initialized.len}\r\n\r\n${initialized}', 'Content-Length: ${call.len}\r\n\r\n${call}'].join('\r\n')
	mut reader := io.new_buffered_reader(reader: FramedReader{ text: stream })
